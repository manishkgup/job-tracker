...
router.post('/', (req, res) => {
  const { company, position, status, date_applied, notes, job_url, location, salary_range } = req.body;
  ...
  const result = db
    .prepare(`
      INSERT INTO jobs (user_id, company, position, status, date_applied, notes, job_url, location, salary_range)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    .run(
      req.user.id,
